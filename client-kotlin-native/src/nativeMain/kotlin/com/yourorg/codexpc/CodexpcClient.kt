...
class CodexpcClient(private val serviceName: String) {
    ...
    fun startWithHandle(req: Request): StartHandle {
        ...
        val f = startInternal(req) { connRef }
        val cancelFun = {
            val c = connRef
            if (c != null) {
                memScoped {
    ...
    fun start(req: Request): Flow<Event> = callbackFlow {
    ...
    private fun startInternal(req: Request, connProvider: () -> xpc_connection_t?): Flow<Event> = callbackFlow {
        var conn: xpc_connection_t? = null
        memScoped {
            conn = xpc_connection_create_mach_service(serviceName, null, 0u)
            // expose to outer
